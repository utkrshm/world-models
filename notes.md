...
- Train the VAE and Memory modules end-to-end, together. Use a joint loss function to train the VAE and Memory together, so that the VAE can learn the dynamics of the game state better.
  - This was not useful; not only did it take very long and complex code with a weighted loss function, it was also not very helpful as after re-normalizing the recon loss and kld to per-frame basis rather than summing over the episode (which was the representation in the RNN dataset), the RNN loss kept increasing, no matter how many epochs I tried it for (1 / 10 / 30). Gonna try the other method now.
- Train them separately, but add a dynamics aware reconstruction component in the final loss, that checks specifically for the presence of dynamic components (in this case, the power pellets, pacman and the ghosts, the things that keep moving through the game run) in the reconstructions.
  - This has to be done because the current reconstruction objective doesn't care about the non-reconstruction of dynamic sprites, that occupy max to max 12x12 space in a 64x64 grid, because the objective right now is to essentially closely mimic the game state being given to it, it doesn't care about the task-relevant parts of the environment, only about the general look of the environment. The authors explicitly state this as a limitation of the original architecture, saying: 
...
**Solution:** Adding a dynamic reconstruction component to the loss helped. Giving it a high weight (lambda_dyn=5.0) helped the model learned the internal representation of the game state decently well; it's not picture perfect, but it is something.
- One thing that could be tried is sort-of scheduling the dynamic reconstruction component such that that becomes more and more important as the training goes on, whereas you reduce the importance of the original reconstruction component with the same scale. So you would first emphasize the model to learn the representation of the background maze, whereas in later training the emphasis would be on the placement of the dynamic sprites. Anyhow, I'm now trying to train the Memory module based on the VAE trained from this.