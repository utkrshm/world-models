...
def train(data_dir, run_name, epochs=1, batch_size=16, num_workers=0, lr=1e-3,
          log_every=1000, num_eval_batches=10, kl_warmup_steps=1000):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in steps_bar:
            ...
            global_step += 1
            beta = min(global_step / kl_warmup_steps, 1.0)
            
            recon, mu, logvar = model(batch)
            _, recon_loss, kld = vae_loss(recon, batch, mu, logvar)
        ...
        print(f"\nEpoch {epoch} training done (step {global_step}) — "
              f"train_loss: {avg_epoch_loss:.4f}  recon: {avg_epoch_recon:.4f}  kld: {avg_epoch_kld:.4f}, beta: {beta:.4f}")

        print("Running full evaluation on test set...")
        test_metrics = evaluate(model, test_dl, device, beta=beta)
...
@torch.no_grad()
def evaluate(model, test_dl, device, max_batches=None, beta=1.0):
